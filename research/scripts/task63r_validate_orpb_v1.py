"""Resume frozen ORPB_V1 validation after Task 63R Alpaca feed diagnosis."""
...
FEED_DIAGNOSTIC = OUT / "feed_diagnostic.json"
...
def load_and_gate() -> tuple[dict[str, Any], dict[str, tuple[pd.DataFrame, pd.DataFrame]]]:
    ...
    write_json(OUT / "uniform_feed_manifest.json", {
        "task": "63R",
        "provider": "Alpaca",
        "feed": "sip",
        "uniform_35_symbol_dataset": feed_uniform,
        "existing_task63_package_was_already_sip": feed_uniform,
        "entire_package_start": "2025-01-24",
        "entire_package_end": "2025-05-05",
        "symbols": universe,
        "source_file_sha256": source_hashes,
        "download_summary_sha256": sha256(DOWNLOAD_SUMMARY),
        "feed_diagnostic_sha256": sha256(FEED_DIAGNOSTIC),
        "mixed_feeds": False,
        "bars_fabricated_or_interpolated": False,
    })
...
def replay(datasets: dict[str, tuple[pd.DataFrame, pd.DataFrame]]) -> pd.DataFrame:
    ...
    write_json(OUT / "replay_manifest.json", {
        "task": "63R", "candidate": "ORPB_V1", "single_replay": True,
...
def analyze(trades: pd.DataFrame) -> dict[str, Any]:
    ...
    (OUT / "validation_summary.md").write_text(
        "# Task 63R — ORPB_V1 Independent Validation #1\n\n"
