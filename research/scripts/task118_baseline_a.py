"""
Task 118 Deliverable A -- exact 39-name V2 baseline under the frozen contract.

Drives the REAL frozen production path (talonx_v2.service.V2Service, via
talonx_research.replay_engine.run_chronological_replay) chronologically over
the Task 116 usable window (2024-09-01 .. 2026-03-31, the SEC Form4 bulk
parquet coverage limit), restricted via a symbol-filtered records_provider to
the frozen 39-name execution-scope manifest (resolved 2026-09-11, see
docs/research/TASK118_INVENTORY.md).

Data source: the historical research parquet + local bar CSVs under
C:\\workspace\\TalonX\\results\\ (frozen, static, read-only artifacts from
Task 95G/107A -- NOT the live production databases). The replay ledger is
written under results/ in THIS worktree only; replay_engine's own
LiveLedgerProtectionError refuses to ever open the live v2_lane.db.
...
STARTING_CASH = 10_000_000.0                 # Task 116's research sizing -- for comparability with
                                              # the broader validated population. NOT the live $300k
                                              # campaign; reported separately.
...
def main() -> int:
    ...
    ledger = OUT / "replay_v2_lane.db"   # under results/ in the RESEARCH worktree -- never the live one
    ...
    rd = res.to_dict()
    (OUT / "baseline_a_replay_result.json").write_text(json.dumps(rd, indent=2, default=str))
    ...
    (OUT / "baseline_a_summary.json").write_text(json.dumps(summary, indent=2, default=str))
