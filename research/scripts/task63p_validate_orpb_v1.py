"""Execute frozen ORPB_V1 with Task 63P per-symbol-session fail-closed readiness."""
...
FEED_DIAGNOSTIC = ROOT / "results/task63r_orpb_v1_feed_remediation/feed_diagnostic.json"
READINESS_FREEZE = OUT / "readiness_freeze.json"
READINESS_MANIFEST = OUT / "readiness_manifest.csv"
PREOUTCOME_FREEZE_COMMIT = "f0f279668f12a8c8fc719659be71cc297a294f17"
...
def load_and_gate() -> tuple[dict[str, Any], dict[str, tuple[pd.DataFrame, pd.DataFrame]]]:
    ...
    feed_diagnostic = json.loads(FEED_DIAGNOSTIC.read_text(encoding="utf-8"))
    readiness_freeze = json.loads(READINESS_FREEZE.read_text(encoding="utf-8"))
    readiness = pd.read_csv(READINESS_MANIFEST, dtype=str)
    ...
    readiness_hashes_match = (
        sha256(OUT / "readiness_protocol.md") == readiness_freeze["readiness_protocol_sha256"]
        and sha256(ROOT / "research/scripts/task63p_readiness.py")
        == readiness_freeze["readiness_code_sha256"]
        and sha256(ROOT / "tests/test_task63p_orpb_readiness.py")
        == readiness_freeze["readiness_tests_sha256"]
    )
    ...
    freeze_is_ancestor = subprocess.run(
        ["git", "merge-base", "--is-ancestor", PREOUTCOME_FREEZE_COMMIT, "HEAD"],
    ...
    feed_uniform = (
        feed_diagnostic["persisted_task63_feed"] == "SIP"
        and feed_diagnostic["implicit_omitted_matches_persisted"]
        and feed_diagnostic["feed_matches"]["sip"]
        and not feed_diagnostic["feed_matches"]["iex"]
        and feed_diagnostic["sip_available"]
    )
    observed_not_ready = data_not_ready_set(readiness)
    readiness_exact = (
        len(readiness) == 2100
        and observed_not_ready == FROZEN_DATA_NOT_READY
        and int((readiness.status == "CLEAN").sum()) == 2094
        and readiness_freeze["correction_frozen_before_outcomes"]
        and not readiness_freeze["validation_started"]
        and not readiness_freeze["outcomes_inspected"]
    )
    write_json(OUT / "validation_source_manifest.json", {
        "task": "63P", "provider": "Alpaca", "feed": "sip",
        "uniform_35_symbol_dataset": feed_uniform,
        "source_file_sha256": source_hashes,
        "download_summary_sha256": sha256(DOWNLOAD_SUMMARY),
        "readiness_freeze_sha256": sha256(READINESS_FREEZE),
        "readiness_manifest_sha256": sha256(READINESS_MANIFEST),
        "bars_fabricated_interpolated_or_filled": False,
    })

    proof = subprocess.run(
        [sys.executable, "-m", "pytest",
         "tests/test_task63p_orpb_readiness.py", "tests/test_orpb_v1.py", "-q",
         "-p", "no:cacheprovider", "--basetemp", str(OUT / "_pytest_gates")],
        cwd=ROOT, capture_output=True, text=True,
    )
    code_proofs_green = proof.returncode == 0 and "16 passed" in proof.stdout
    ...
    for window in manifest["windows"]:
        ...
        evaluation_raw = date_slice(regular, evaluation_dates)
        window_readiness = readiness[readiness.window == name].copy()
        evaluation = clean_evaluation_rows(evaluation_raw, window_readiness)
        ...
        for symbol in universe:
            ...
            raw_symbol = evaluation_raw[evaluation_raw.symbol == symbol]
            clean_symbol = evaluation[evaluation.symbol == symbol]
            warm_dates_observed = sorted(
                warm_symbol.timestamp.dt.tz_convert(ET).dt.strftime("%Y-%m-%d").unique()
            )
            eval_dates_observed = sorted(
                raw_symbol.timestamp.dt.tz_convert(ET).dt.strftime("%Y-%m-%d").unique()
            )
            opening = opening_bucket_coverage(raw_symbol, evaluation_dates)
            expected_not_ready = sorted(
                window_readiness[
                    (window_readiness.symbol == symbol)
                    & (window_readiness.status == "DATA_NOT_READY")
                ].session.tolist()
            )
            opening_matches_frozen_readiness = sorted(opening["failures"]) == expected_not_ready
            report = quality[symbol]
            per_symbol[symbol] = {
                "warmup_rows": len(warm_symbol),
                "raw_evaluation_rows": len(raw_symbol),
                "eligible_evaluation_rows": len(clean_symbol),
                ...
                "data_not_ready_sessions": expected_not_ready,
                "opening_failures_match_frozen_readiness": opening_matches_frozen_readiness,
        ...
        window_pass = all(
            ...
            and item["opening_failures_match_frozen_readiness"]
        ...
        dnr_count = int((window_readiness.status == "DATA_NOT_READY").sum())
        window_gates[name] = {
            "gate_passed": window_pass,
            "symbols_present": int(evaluation_raw.symbol.nunique()),
            "warmup_sessions": len(warmup_dates),
            "evaluation_sessions": len(evaluation_dates),
            "expected_symbol_sessions": len(universe) * len(evaluation_dates),
            "clean_symbol_sessions": len(universe) * len(evaluation_dates) - dnr_count,
            "data_not_ready_symbol_sessions": dnr_count,
            "warmup_rows": len(warmup),
            "raw_evaluation_rows": len(evaluation_raw),
            "eligible_evaluation_rows": len(evaluation),
            "opening_range_failures": [
                {"symbol": symbol, "sessions": item["data_not_ready_sessions"]}
                for symbol, item in per_symbol.items() if item["data_not_ready_sessions"]
            ],
            "unexpected_readiness_failures": [
                symbol for symbol, item in per_symbol.items()
                if not item["opening_failures_match_frozen_readiness"]
            ],
        ...
        datasets[name] = (warmup, evaluation)

    latest_local_date = regular.timestamp.dt.tz_convert(ET).dt.date.max().isoformat()
    global_gates = {
        "readiness_correction_commit_frozen_before_replay": freeze_is_ancestor,
        "corrected_readiness_exact_no_broader_problem": readiness_exact,
        "readiness_code_protocol_test_hashes": readiness_hashes_match,
        ...
        "uniform_alpaca_sip_feed_proven": feed_uniform,
        "code_level_causality_isolation_parity_proofs": code_proofs_green,
        "no_future_or_extra_data": latest_local_date <= "2025-05-05",
        "all_windows_corrected_coverage_quality_readiness": all(
            item["gate_passed"] for item in window_gates.values()
        ),
        "no_fabrication_interpolation_or_fill": True,
    }
    evaluation_bars = sum(len(item[1]) for item in datasets.values())
    gate_payload = {
        "task": "63P",
        "base_commit": BASE_COMMIT,
        "preoutcome_readiness_freeze_commit": PREOUTCOME_FREEZE_COMMIT,
        "frozen_implementation_fingerprint": frozen_fingerprint,
        "provider": download.get("provider"),
        "resolved_feed": "sip" if feed_uniform else "unresolved",
        "download_summary_sha256": sha256(DOWNLOAD_SUMMARY),
        "source_file_sha256": source_hashes,
        "readiness": {
            "expected_symbol_sessions": 2100,
            "clean_symbol_sessions": 2094,
            "data_not_ready_symbol_sessions": 6,
            "clean_coverage_percent": 100.0 * 2094 / 2100,
            "exact_data_not_ready": readiness_freeze["exact_data_not_ready"],
        },
        "total_raw_bars": len(frame),
        "total_regular_session_bars": len(regular),
        "runtime_estimate": {
            "eligible_evaluation_regular_bars": evaluation_bars,
            "basis": "one shared ORPB state-machine pass over fail-closed eligible regular-session rows",
            "estimated_minutes_range": "1-5",
        },
        "code_proof": {
            "command": (
                "python -m pytest tests/test_task63p_orpb_readiness.py "
                "tests/test_orpb_v1.py -q -p no:cacheprovider"
            ),
...
def replay(datasets: dict[str, tuple[pd.DataFrame, pd.DataFrame]]) -> pd.DataFrame:
    ...
    write_json(OUT / "replay_manifest.json", {
        "task": "63P", "candidate": "ORPB_V1", "single_replay": True,
...
def analyze(trades: pd.DataFrame) -> dict[str, Any]:
    ...
    correctness = {
        ...
        "no_data_not_ready_candidate_order_or_trade": trade_session_keys.isdisjoint(
            FROZEN_DATA_NOT_READY
        ),
        "orpb_alpha_fingerprint_unchanged": implementation_fingerprint() == EXPECTED_FINGERPRINT,
    ...
    summary = {
        ...
        "orpb_alpha_fingerprint_unchanged": True,
        "readiness_correction_frozen_before_outcomes": True,
        "expected_symbol_sessions": 2100,
        "data_not_ready_symbol_sessions": 6,
        "clean_coverage_percent": 100.0 * 2094 / 2100,
        "no_fabrication_interpolation": True,
        "trades": len(trades),
...
def write_blocked_outputs(gates: dict[str, Any]) -> None:
    ...
    blocker = {
        ...
        "resolved_alpaca_feed": "SIP",
        "uniform_35_symbol_dataset": True,
        "opening_range_completeness": False,
        "task63_history_preserved": True,
        "provider_definition_correction_not_strategy_tuning": True,
        "reason": (
    ...
    summary = {
        ...
        "classification": "VALIDATION_BLOCKED",
        "resolved_alpaca_feed": "SIP",
        "uniform_35_symbol_dataset": "PASS",
        "opening_range_completeness": "FAIL",
        "trades": None,
    ...
    (OUT / "validation_summary.md").write_text(
        ...
        "The existing package and explicit SIP refetch match; explicit IEX does not. This is a "
        "provider/data-definition correction before outcome access, not strategy tuning.\n\n"
