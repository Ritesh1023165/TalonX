...
~~The only correction is the timestamp label (erratum E1 below).~~ **Erratum E1 is RETRACTED** (see below); the original Session 02 forensic times were correct. No conclusion changes.

## 3. Findings

| ID | Finding | Classification |
|---|---|---|
| F1 | **SUPERSEDED.** The original claim ("`accepted_at_utc` holds SEC's New York wall-clock time labelled UTC") was only partly true. SEC's submissions feed serves a fresh filing's `acceptanceDateTime` as New York wall-clock labelled `Z`, then rewrites it to true UTC about 5.5 h later. TalonX stored whichever version it saw, so the field is **mixed** (397/425 true UTC, 28/425 ET in the audited sample). Its "no V2 decision impact" conclusion was also **wrong**: V2 derived the filing date from `accepted_at_utc.date()` because `filing_date` was never populated. This was reclassified `RELEASE_BLOCKING_ADMISSION_DEFECT` and is fixed in `../v2_sec_filing_date_release_fix/`. | SUPERSEDED → fixed (see `v2_sec_filing_date_release_fix/`) |
...
**ERRATUM E1 RETRACTED** (2026-09-22). E1 read: ~~"SEC-accepted 2026-09-17T11:00:24Z (07:00 ET)" should read 11:00:24 ET = 15:00:24Z; receipt lag 4 d 3 h 42 m~~.

*Reason for retraction:* E1 assumed `accepted_at_utc` always stored SEC New York wall-clock time mislabelled UTC. The raw SGML `<ACCEPTANCE-DATETIME>` (Eastern) proves the field is mixed, depending on SEC's feed state at ingest. ADC was ingested 4 days after acceptance, when SEC already served true UTC.

**The original Session 02 ADC forensic UTC value was correct:** SEC accepted 2026-09-17 **07:00:24 America/New_York = 11:00:24 UTC**. TalonX receipt was 2026-09-21 18:42:40 UTC, a delay of about **4 d 7 h 42 m**. The ABCL activating filing was accepted **12:04:25 ET (16:04:25 UTC)**, not "16:04:25 ET". ADC stays `LEGITIMATE_TIMING_REJECTION` / `MISSED_DUE_TO_INGESTION_DOWNTIME`, and neither the decision nor the Session 02 verdict changes. Full evidence: `SEC_TIMESTAMP_IMPACT_AUDIT.md` and `../v2_sec_filing_date_release_fix/TIMESTAMP_SEMANTICS.md`.

## 4. Observability certainty (Task M)

| Level | Items |
|---|---|
| **PROVEN** | The 39-symbol universe (companion logs, both sessions). The code-P set and all Form 4 counts: the store matches SEC EDGAR for all 39 CIKs (95 filings at SEC since 09-16 = 89 in store + 6 accepted after the last poll/stop, with 0 errors), and the 6 post-stop filings are non-P. The 2 clusters (the frozen engine matches the ledger IDs). Ledger state: 0 intents, 0 V2 outbox rows, 0 positions or trades, $100,000 cash. TalonX receipt timestamps. Insider ingestion first ran 09-04. 0 filings ingested 09-16..09-20. SEC timestamps: see the F1 supersession (mixed; ET-labelled only while fresh). Liquidity, sizing and hypothetical P&L on FINAL SIP bars. ADC's only dividend ex-date is before entry. |
