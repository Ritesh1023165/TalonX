...
| 9 | Telegram and dashboard experience | Not yet conducted. **This is NOT "Account Ledgers & Risk"** — that topic maps to Session 10 below, where Session 8's own account-ledger/risk carry-forward items are recorded instead (see `DECISION_LOG.md` Session 8's closing note for the full mapping). Session 9 itself still candidate-destinations `S5-30` (pre-market lockout window, undefined), Session 6's `S6-13`/`S6-14` (capacity-independent alerts, expiry-update UI) and `S6-11` (four-category diagnostic surfacing), and Session 7's `S7-23`/`S7-24` (five-dimension coverage surfacing) and `S7-20`'s per-stock mute UI. |
| 10 | Paper accounting, costs and risk | Not yet conducted. Receives `S3-25` (cross-strategy exposure enforcement policy, deferred from Session 3), and now Session 8's carried-forward account-ledger/risk items: separate strategy/execution-mode accounts (`S3-13`, V2 detail `S8-12`-`S8-14`), cash vs. reservations vs. invested capital (`S8-12`), new-campaign capital defaults (`S3-12`, unchanged), same-stock cross-strategy exposure (`S3-25`), corrections/deposits/return attribution (`S2-11`, `S7-14`, `S8-11`), account blocks and auditable release (`S8-13`/`OPS-012`), and combined portfolio views without double-counting experimental capital (`S3-13`). |
| 11 | Operation, stop/start and recovery | Not yet conducted. Shares `S3-28` (mute controls, pending-intent handling) with Session 4; candidate destination for `S5-30` alongside Session 9, for Session 6's `OPS-007` (intraday EOD-flatten durable recovery) and `S6-22`/`S6-23`'s cross-restart persistence requirements, and now Session 8's `OPS-012` (V2 `EXIT_UNRESOLVED` account-block enforcement) and `S8-26` (holiday/early-close boundary test coverage). |
| 12 | Technical validation, usefulness and economic evidence | Not yet conducted. Receives `S3-26` (experiment evaluation criteria/evidence sufficiency, deferred from Session 3), is a candidate destination for `S5-31`'s 2-year-replay-feasibility audit, `S6-25` (numerical spread/slippage/fee assumptions), and now `S7-25`/`S7-26` (numerical freshness windows and event-specific materiality thresholds, each requiring a bounded historical-filing review) — all prerequisites for authorizing any future experiment or implementation of Session 6/7's approved targets. |
...
deferrals (`S7-18`, `S7-25`, `S7-26`), and Session 8's own deferrals
(`S8-25`, `S8-26`) follow the same pattern — see `DECISION_LOG.md`
Session 3/5/6/7/8's "Explicit deferrals" sections.
